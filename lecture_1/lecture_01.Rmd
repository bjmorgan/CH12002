...
Chemical kinetics tells us how *fast* a reaction proceeds. Together with thermodynamics, it provides a complete picture of chemical change, whether in laboratory settings, industrial applications, or natural systems.
...
Answering these questions requires us to understand how reaction rates relate to concentrations.
...
For our A + 2B → C example, with $\nu_\mathrm{A} = 1$, $\nu_\mathrm{B} = 2$, and $\nu_\mathrm{C} = 1$, this gives:
...
Why do most reactions slow down as they progress? At the start, plenty of reactant is available and molecular encounters are frequent, so the reaction proceeds quickly. As reactant is consumed, fewer molecules remain, encounters become less frequent, and the rate falls. This suggests a general principle: the rate of a reaction depends on the concentrations of the reactants. The mathematical expression that captures this dependence is called a **rate law**.
...
Not all reactions follow a simple rate law. A classic example is the formation of hydrogen bromide:
