...
The rate law contains three concentration terms, giving an overall order of three. The stoichiometry is consistent with a single termolecular step — two NO molecules and one O~2~ molecule coming together simultaneously. But the probability of three molecules arriving at the same point at the same instant is negligibly small. A termolecular elementary step cannot account for the observed rate. The reaction also has an unusual temperature dependence: the rate *decreases* with increasing temperature. For a single elementary step, we would expect the opposite: higher temperatures mean faster molecular motion and more energetic collisions, so a larger fraction of molecules have enough energy to react. Any proposed mechanism must account for both observations.
