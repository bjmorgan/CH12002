...
Throughout this course, the steady-state approximation has been one of our most powerful tools: we set $\mathrm{d}[\mathrm{I}]/\mathrm{d}t = 0$ for a reactive intermediate and solve the resulting algebraic equation. We applied it to derive the Lindemann mechanism, the Michaelis--Menten equation, and the H~2~ + Br~2~ rate law. Each time, it worked beautifully. But *why* does a reactive intermediate reach a steady state? And are there circumstances where it does not?
...
The first term, $k_1[\mathrm{A}]$, is the rate at which B is formed. Early in the reaction, when $[\mathrm{A}]$ has not changed much, this is approximately constant. The second term, $k_2[\mathrm{B}]$, is the rate at which B is consumed — and it is proportional to $[\mathrm{B}]$ itself. This proportionality is the key to the steady state.
...
(\#eq:feedback-consecutive)
\end{equation}

Because $k_2 > 0$, the feedback parameter $\phi$ is negative. This is why B reaches a steady state: the negative feedback ensures that the intermediate population is always pulled back towards equilibrium with its surroundings.
...
This is the steady state we have relied on throughout the course. The system is self-regulating: if $[\mathrm{I}]$ drifts above $[\mathrm{I}]_\mathrm{ss}$, the negative feedback pulls it back down. The timescale for reaching the steady state is $\tau = 1/|\phi|$. When $|\phi|$ is large — meaning the processes that consume I are fast — the steady state is reached quickly, $[\mathrm{I}]$ remains low, and the SSA is an excellent approximation. This is precisely the condition we identified in [Lecture 5](#ssa): the intermediate is consumed much faster than it is formed.
...
The system approaches this value with a timescale $\tau = 1/|\phi| = 1/20 = 0.05\,\mathrm{s}$. If $[\mathrm{R}^{\bullet}]$ starts at zero, it rises towards $0.025\,\mathrm{M}$. If it starts above this value, the negative feedback pulls it back down. Both trajectories converge on the same steady state.
...
A mixture of hydrogen and oxygen can sit indefinitely at room temperature, yet a single spark triggers a violent explosion. The same molecules, the same elementary steps — but two completely different outcomes. The feedback framework tells us why: everything depends on whether $\phi$ is positive or negative, and for the H~2~/O~2~ system, the sign of $\phi$ changes with pressure in a remarkable way.
...
To see the net effect, follow one $\mathrm{H}^{\bullet}$ through a complete cycle. Step 1 produces $\mathrm{OH}^{\bullet}$ and $\mathrm{O}^{\bullet\bullet}$. Step 2 converts $\mathrm{O}^{\bullet\bullet}$ to $\mathrm{OH}^{\bullet} + \mathrm{H}^{\bullet}$. Step 3 converts each $\mathrm{OH}^{\bullet}$ to $\mathrm{H}^{\bullet}$. The net result: one $\mathrm{H}^{\bullet}$ produces three new radicals — a net gain of two per cycle. Writing the feedback equation for the total radical population, with the first (rate-limiting) branching step controlling the branching rate:

$$\phi = 2k_\mathrm{b}[\mathrm{O_2}] - k_\mathrm{term}$$

The factor of 2 reflects the net gain of two radicals per branching cycle. Whether this is positive or negative depends on $k_\mathrm{term}$ — and here the story becomes interesting, because the dominant termination mechanism changes with pressure.
